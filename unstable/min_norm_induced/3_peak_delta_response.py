...
for subject in ['2001', '2003', '2008', '2009', '2014']:
    ...
    for session in sessions:
        ...
        # save pseudo T
        pseudoT_left_fname = deriv_path.basename + "-left_stim_delta"
        pseudoT_left.save(op.join(out_folder, pseudoT_left_fname), overwrite=True)
        pseudoT_right_fname = deriv_path.basename + "-right_stim_delta"
        ...
        if plot_timecourse:
            ...
            peak_timecourse = source_epochs_filt.average(picks="all").apply_baseline(baseline)
            
            plt.figure()
            plt.plot(peak_timecourse.times, np.abs(peak_timecourse.get_data()[0]), color="black")
            plt.ylabel("Oscillatory Power (A.U)")
            plt.xlabel("Time (s)")
            plt.title(new_label.name)
        
        # save peak source epoch
        lh_source_fname = deriv_path.basename + "-lh_delta_source.fif"
        ...
        # save peak source epoch
        rh_source_fname = deriv_path.basename + "-rh_delta_source.fif"
