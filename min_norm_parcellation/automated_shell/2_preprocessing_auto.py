...
if len(argv) != 4:
    print("Please pass the subject, session and date, e.g. 2_preprocessing_auto 2001 01N 20230616")
    exit()
...
run = '01'
suffix = 'meg'
date = str(argv[3])  # for empty room noise (needed for minimum-norm estimate)
...
event_plot = mne.viz.plot_events(events)
event_plot.savefig(op.join(figs_dir, deriv_path.basename + "-events"))
...
head_movement_fig.savefig(op.join(figs_dir, deriv_path.basename + "-movement"))
...
psd_plot = data.plot_psd(fmax=45, picks='mag')
psd_plot.savefig(op.join(figs_dir, deriv_path.basename + "-psd"))
...
# plot diagnostics
ica_plots = ica.plot_properties(data, picks=ica.exclude)
for p, plot in enumerate(ica_plots):
    plot.savefig(op.join(figs_dir, deriv_path.basename + "-ICA" + str(p)))
